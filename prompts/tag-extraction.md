...
2. **每个标签必须附带溯源**：
   ...
   - `speaker`：原句的说话人（"customer"、"consultant" 或 "unknown"）
   - `confidence`：你对该标签的置信度（high / medium / low）
3. **客户说的话权重高于咨询师转述**。咨询师的引导性提问不能作为客户意向的证据。`unknown:<id>` 只是 ASR 声音分组，不能假定其为客户或咨询师；只有句子本身明确是第一人称需求、预算、顾虑或个人信息时才可提取，并把 `speaker` 输出为 `unknown` 且 `confidence` 不高于 `medium`。
...
以下是一段医美面谈对话转写。`consultant`=已验证咨询师，`customer`=已验证客户，`unknown:<id>`=未验证的 ASR 分组。请按照规则提取客户标签，严格输出 JSON。
