...
function AlertsTab({ onError }: { onError: (msg: string) => void }) {
  ...
  const fetchRules = useCallback(async () => {
    try {
      const res = await apiFetch("/api/platform/monitoring/alerts/rules")
  ...
  const fetchEvents = useCallback(async () => {
    try {
      ...
      const res = await apiFetch(`/api/platform/monitoring/alerts/events?${params.toString()}`)
  ...
  const handleDeleteRule = async (rule: AlertRule) => {
    ...
    try {
      const res = await apiFetch(`/api/platform/monitoring/alerts/rules/${rule.id}`, { method: "DELETE" })
  ...
  const handleToggleRule = async (rule: AlertRule) => {
    try {
      const res = await apiFetch(`/api/platform/monitoring/alerts/rules/${rule.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleEventAction = async (event: AlertEvent, action: "acknowledge" | "resolve" | "ignore") => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/platform/monitoring/alerts/events/${event.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleResolve = async () => {
    ...
    setActionLoading(true)
    try {
      const res = await apiFetch(`/api/platform/monitoring/alerts/events/${selectedEvent.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
...
function TicketsTab({ onError }: { onError: (msg: string) => void }) {
  ...
  const fetchTickets = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/platform/monitoring/tickets?${params.toString()}`)
  ...
  const fetchTicketDetail = async (id: string) => {
    ...
    try {
      const res = await apiFetch(`/api/platform/monitoring/tickets/${id}`)
  ...
  const handleSendReply = async () => {
    ...
    try {
      const res = await apiFetch(`/api/platform/monitoring/tickets/${selectedTicket.id}/messages`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleUpdateStatus = async (status: string) => {
    ...
    try {
      const res = await apiFetch(`/api/platform/monitoring/tickets/${selectedTicket.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ status }),
      })
