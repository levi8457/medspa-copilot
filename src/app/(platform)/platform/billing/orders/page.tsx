...
export default function OrdersPage() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/platform/orders?${params.toString()}`)
  ...
  const handleMarkPaid = async (order: Order) => {
    ...
    try {
      const res = await apiFetch(`/api/platform/orders/${order.id}`, {
        method: "PATCH",
...
function CreateOrderModal({
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        ...
        const orgJson: ApiResponse<{ organizations: OrganizationOption[]; pagination: unknown }> = await orgRes.json()
        const planJson: ApiResponse<PlanOption[]> = await planRes.json()
        if (orgJson.success && orgJson.data?.organizations) {
          setOrganizations(orgJson.data.organizations)
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      const res = await apiFetch("/api/platform/orders", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          orgId: form.orgId,
