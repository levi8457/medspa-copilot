...
export default async function AdminCustomerDetailPage({
  ...
  const customer = await prisma.customer.findUnique({
    ...
    include: {
      tags: true,
      audioRecords: { orderBy: { createdAt: "desc" }, take: 5 },
      followUpPlans: {
        include: { tasks: { orderBy: { scheduledDate: "asc" }, take: 10 } },
        orderBy: { createdAt: "desc" },
        take: 3,
      },
      timelineEvents: { orderBy: { occurredAt: "desc" }, take: 20 },
      consumptionRecords: { orderBy: { consumedAt: "desc" }, take: 10 },
      consultant: { select: { id: true, name: true, phone: true } },
    },
  })

  if (!customer || customer.orgId !== session.user.orgId) notFound()
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold text-[var(--foreground)]">客户详情</h1>
          </div>
          <div className="flex items-center gap-3">
            ...
            <Link
              ...
              className="flex items-center gap-2 px-4 py-2 bg-[var(--primary)]/20 text-[var(--primary)] rounded-lg hover:bg-[var(--primary)]/30 transition-colors"
            >
              ...
              {customer.consumptionRecords.length > 0 ? (
                <div className="space-y-2">
                  {customer.consumptionRecords.slice(0, 3).map((record) => (
                    <div key={record.id} className="flex items-center justify-between text-sm py-2 border-b border-[var(--border)] last:border-0">
                      <span className="text-[var(--foreground-secondary)]">{new Date(record.consumedAt).toLocaleDateString()}</span>
                    ...
                    return (
                      <div key={plan.id} className="space-y-3">
                        ...
                        {plan.tasks.slice(0, 3).map((task) => (
                          <div key={task.id} className="flex items-center justify-between p-3 rounded-lg bg-[var(--background)]/50 border border-[var(--border)]">
                            <div>
                              <p className="font-medium">{task.goal || "跟进任务"}</p>
                              <p className="text-sm text-[var(--foreground-secondary)]">{new Date(task.scheduledDate).toLocaleDateString()}</p>
                            </div>
                            <span className={`px-2 py-0.5 rounded text-xs ${
                              task.status === "done" ? "bg-[var(--success)]/20 text-[var(--success)]"
                      ...
                      <div key={event.id} className="relative pl-10">
                        ...
                        <div className="p-3 rounded-lg bg-[var(--background)]/50 border border-[var(--border)]">
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <span className="text-xs text-[var(--foreground-secondary)]">{new Date(event.occurredAt).toLocaleString()}</span>
                          </div>
                          {event.content && <p className="text-sm text-[var(--foreground-secondary)]">{event.content}</p>}
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {customer.audioRecords.length > 0 ? (
                <div className="space-y-4">
                  {customer.audioRecords.map((record) => (
                    <div key={record.id} className="p-4 rounded-lg bg-[var(--background)]/50 border border-[var(--border)]">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                        ...
                        <span className={`px-2 py-0.5 rounded text-xs ${
                          record.status === "done" ? "bg-[var(--success)]/20 text-[var(--success)]"
                            : record.status === "pending" ? "bg-[var(--warning)]/20 text-[var(--warning)]"
                            : "bg-[var(--danger)]/20 text-[var(--danger)]"
                        }`}>
                          {record.status === "done" ? "已解析" : record.status === "pending" ? "待解析" : "解析失败"}
                        </span>
                      </div>
                      {record.status === "done" && (
